...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "v1-g4-face-models.json"
...
def prepare_model(model: dict, *, models_dir: Path, offline: bool) -> dict:
    ...
    if not pinned:
        with tempfile.TemporaryDirectory(prefix="kangshield-v1-g4-face-") as temp:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download and verify the pinned V1-G4 face model weights"
    )
...
def main() -> int:
    ...
    if config.get("schema_version") != "1.0":
        raise ValueError("unsupported V1-G4 face model config schema")
    results = [
