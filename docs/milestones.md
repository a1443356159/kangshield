...
- [ ] 在干净提交上完成 L40 双 variant 对比。
- [ ] Review 横卧覆盖提升、fall-01 关键点质量、性能和许可证后决定是否进入 V2 候选。

设计见 [V1-M3 姿态模型对比](v1-m3-pose-model-comparison.md)。姿态切片完成不等于整个 V1-M3 Done；ASR 对照和睡眠字段路线仍需在 V1-R1 前形成采用/候选/放弃结论。
