...
| V1-M1 设备能力探测 | 07-25～07-28 | In progress | 摄像头/睡眠仪能力矩阵、有界流采集/重复开流/故障识别/session supervisor、API 样例、原始样本 | 五个接收、恢复与时长接缝 E1 已关闭；每项目标设备能力仍须有真实调用或明确“不开放/待确认”证据 |
...
| V1-M1 流会话 Supervisor 与受控恢复 | `6a68371` / `d9e9daa` | `origin/main` | 2026-07-23 已验证 | 独立 segment/raw、start/finish/gap/interruption ledger、30 分钟硬门、全健康 3/3 session 与同 endpoint ready→503→ready、REV-028；只关闭 E1 外部重开接缝，不代表同连接/非自愿断流恢复、长稳、RTSP/packet loss 或 C6c |
| V1-M1 Stream Session wall/media 双时长门加固 | `ab1f366` / `508c667` | `origin/main` | 2026-07-30 已验证 | 父级 ready media 重算、wall/media 独立 gate、30 分钟双声明/双观测契约正反例、两条 clean E1 与 REV-029；不代表实际运行 30 分钟、C6c 或 RTSP 长稳 |
...
- [x] long-run gate 加固：`stream-session-v0.2.0` 从 segment 重算累计 ready media，将 wall/media 声明与观测分门；clean E1 与 30 分钟契约正反例通过，实际 30～60 分钟运行仍未执行。
- [ ] C6c 真实 E2/E3 证据。
...
- [x] 在干净提交 `ab1f366` 上将长稳升级为 wall/ready-media 双时长门：健康 run 显式声明/实际 ready media 为 1,500/2,550 ms，恢复兼容 run 按专用 gate 通过；30 分钟正反例只验证契约，未替代实际长稳。
