...
| V1-R1 探索 Review | 08-08～08-09 | In progress | V1 结论、淘汰项、V2 输入清单 | E1 决策账本已冻结；真机、许可证和负责人硬门关闭后验收 |
...
采集规程见 [V1-M2c 目标设备样本与时间基](v1-m2c-device-sample-protocol.md)。模板完成不提升设备证据等级；真实样本和接口响应仍未到位。
...
预 Review 见 [V1-R1 探索收敛与 V2 输入清单](v1-r1-exploration-review.md)。E1 决策账本完成不等于 V1-R1 Done；真机、最终姿态分发路线和责任人仍是验收门。
