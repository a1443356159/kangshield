...
- **协议指标**（本节 C02/C03 步速、C04 5xSTS、C13/C14 转身的协议化版本）：只在开发集 A 与 held-out B 采集时运行，作用是提供带人工参考的"标准答案"，用于训练和验证被动事件检测器；不进生产长程库。
- **日常被动指标**：生产形态。居家环境不要求老人做任何测试动作、不依赖重复标定，从日常活动的事件流中取数，进长程记忆库做 L1 自比基线。包括 `gait_speed_normalized`、`gait_cadence`、`sts_transition_duration`、`turn_duration`。
...
日常被动版 `gait_speed_normalized`（单位 `leg_lengths/s`，DEC-017）：

- 用被测者自身腿长做尺子：踝点位移 ÷ 同帧腿长像素 ÷ 时间；透视缩放随深度同步作用于分子分母，除法约去，免单应标定、免布场，适应居家场景差异与装机变更；
- 对应生物力学无量纲步速（Froude 类），只用于自比，永远不输出跨人绝对值；`gait_speed`（m/s）在完整标定前保持 `not_assessable`；
- 质量门：全身关键点可见率、行走朝向与主轴一致性（偏航过大降质量或不可评估）、腿长估计稳定性；遮挡、宽松衣物致腿长不可测时 fail-closed；
- 验证：开发集 A 仍拍 3 m/5 m 走道与人工计时，检验归一化步速的重测一致性及与真实步速的相关性；不达标则退回走道通过时间方案。
...
日常被动版 `sts_transition_duration`（单位 s，DEC-017）：生产环境不做协议测试，用同一状态机的子集检测日常每次自然"坐→站"转移（沙发、餐椅、床沿），输出单次转移时长；单样本噪声大（椅型、扶扶手、借力差异），按日取中位数后进 L1 基线，样本量每天 10+，趋势敏感性优于每周一次协议测试。椅面/髋膝不可见或转移中断的事件不计入。协议版 5xSTS 仅用于开发集验证检测器边界准确性。
...
日常被动版 `turn_duration`（单位 s，DEC-017）：检测日常活动中的自然转身事件（肩髋朝向角变化 ≥150° 计为一次），输出事件耗时分布，按日中位数进 L1 基线；协议版 C13/C14 仅用于验证事件检测与边界。
