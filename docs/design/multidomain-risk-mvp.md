...
- 心理健康：合并本人每月填写的 [WHO-5](https://www.who.int/publications/m/item/WHO-UCN-MSD-MHE-2024.01) 与日间出现率、活动、语言互动和已确认睡眠规律。行为侧 28 天基线至少 7 个日期、每天 3 个合格片段；`|z|>=1.5`/`2.5` 为轻/重变化。一个轻变化为 1，两个轻或一个重为 2，两个重或 2 级连续三个相邻自然日为 3，无变化为 0；缺失或低覆盖日期不能桥接成连续三天。WHO-5 五项各 0–5 分，原始分低于 13 时按 WHO 建议进一步评估的界线使本域至少为 2；原始分 13–25 只提供本月未命中该界线的证据，不降低行为规则已经给出的等级。两路取较高等级。本月未填时不推测答案，行为基线不足时仍为 `null`；不做表情或声学情绪识别，不把问卷解释成临床诊断。
...
owner 离线导出与仓库 [`showcase/index.html`](../../showcase/index.html) 复刻部署后 demo 的主要信息层级：三域问题筛查、28 天趋势、个人基线、风险相关语音、月度 WHO-5、近期提醒和内嵌条款均在单文件中呈现。静态问卷只在当前浏览器页面计算筛查提示，不联网、不持久化，也不改变导出时冻结的风险等级；正式写入、复核和媒体播放仍只在 localhost 服务中完成。public 离线导出继续移除身份、问卷答案、对话、备注、路径和精确事件时间。
