...
证据快照：截至 `f5bb761` 的 V1-M1～V1-M3 报告、许可证 fail-closed 修正 `f4aa1c5`、G4 离线特征实现 `782026b`、CAUCAFall ADL 压力实现 `336bbe9`，以及 Keypoint R-CNN 候选实现 `eae5f56` / G4 派生接入 `d956203`
...
| R1-M03 | box-only 横卧/下降/静止 + keypoint quality gate | Conditional implementation input | URFD RTMPose lying 为 17 horizontal；CAUCAFall RTMPose 有 17 个 horizontal box-only；Keypoint R-CNN 又提供 5 个 gate-passed + torso-horizontal 的 no-fall 反例 | 在 C6c 跌倒正负样本和空房/家具/躺床/宠物/多人 held-out 集上冻结组合规则、多人策略与事件指标 |
...
| R1-M08 | TorchVision Keypoint R-CNN COCO_V1 | Conditional fallback；not selected | M2b 163/170、lying 21/21、RTF 0.100451；但 lying gate 仅 4/21；CAUCAFall 621/661 且有 46 torso-horizontal no-fall 帧 | C6c held-out 反超、横卧关键点门改善，并关闭 COCO/ImageNet/weight 分发 Review 后才可重开主候选决定 |
