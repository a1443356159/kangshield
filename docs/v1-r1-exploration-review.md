...
| Gate | 必须取得的证据 | 失败时的降级 |
|---|---|---|
| G1 C6c 能力 | 脱敏能力集、直播/回放/抓图调用和一段原始媒体；E1 HTTP 采集/重复开流 gate 只作为调用后的接收端 | 演示只允许受控流/文件回放，不声称实时萤石接入 |
