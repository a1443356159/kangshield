...
## REV-030 七维指标可行性修正与命名规范 Review

- 日期：2026-08-02
- 状态：Accepted
- 参与人：项目组
- 评审范围：《监测方案》七维指标与四类风险的可采集性、指标命名、验证口径、设备边界确认
- 输入材料：外部指标可行性评审（2026-08 提供，含摄像头/语音/睡眠仪逐条指标核对、修订版 MVP、三层验证建议和两个落地硬门槛）、[信息采集探索](v1-information-acquisition.md)、[设备能力矩阵](device-capability-matrix.md)、[V1-R1 收敛清单](v1-r1-exploration-review.md)

### 发现

1. 外部评审的排除/降级清单与 V1-R1 已冻结决策高度一致：HRV 频域、血氧、AHI、体温、血压、COP、微表情、社交元数据、心理症状被动识别均不可由当前设备支持，属于对既有 Exclude/Defer 的独立确认。
2. 新增硬边界：谐波比 HR/iHR 是躯干 IMU 频谱指标，视频不得沿用其定义和文献阈值；IV、IS、RA、M10、L5 是非参数 24 小时节律指标，需 6～7 天接近全天候连续活动计数，床旁设备加单房间摄像头覆盖不了白天外出活动；余弦拟合（Cosinor）只产生 mesor、振幅和峰相位，不能冒充非参数指标。
3. 步态真实尺度需要固定机位、锁定云台、地面单应性标定和 3～4 米固定步行区；25/30 fps 对双支撑等短时间相的量化粒度约 40 ms，自然走道几步场景不稳定。
4. 语音指标应以每周固定 60～120 秒标准化任务为主要采集形态；Jitter/Shimmer/HNR/共振峰仅限近讲标准化录音；输出前必须确认说话人为老人本人且 SNR 合格。
5. 跌倒事件结论必须报告灵敏度、漏报率和每户每天误报次数，只报"准确率"不合格。
6. 诈骗口径需从单关键词升级为"关键词+语义意图+异常行为"三证据融合，任何单一关键词不得直接触发告警；"隐瞒、眼神躲闪、突然注重打扮"等高伦理风险判断禁止作为证据。
7. 外部评审基于的 PPT 设备清单（H3、C6Wi-4K、CS-EP-SDHY1）与本仓库冻结边界不同；项目组确认维持 CS-C6c-V101-1J4WF + CS-EP-SDNL1，指标修正按现有设备适配。
8. 隐私口径需统一为端侧完成 VAD/说话人确认/ASR/意图判定，只上传事件类型、置信度、时间和质量分；敏感个人信息需单独同意与个人信息保护影响评估，人脸处理遵循 2025-06-01 施行的《人脸识别技术应用安全管理办法》。

### 决定

1. 修正 [信息采集探索](v1-information-acquisition.md) 第 3 节七维映射与 3.1 Backlog：步态真实尺度加固定步行区/标定/连续周期条件；IV/IS/RA/M10/L5 由 P2 移入暂不可得；面部 AU 在高位远场默认不支持；新增"视频测量冒充项"和"心理症状被动识别"两类暂不可得。
2. 冻结七条指标命名规范（3.1.1）："肌力"→"坐站和功能活动表现"、"COP 面积/速度"→"视觉躯干/重心投影摇摆代理量"、"情绪识别"→"面部/语音情绪线索"、"抑郁/认知诊断"→"需进一步量表或临床评估的风险提示"、"诈骗识别"→"疑似高风险交互事件"、"单/双支撑线"→"单/双支撑时间"、"步线长度"→"步长/步幅"。
3. 冻结三层验证口径（测量层 MAE/Bland–Altman/ICC、事件层召回/精确/每户每天误报/延迟、风险层只报基线相关性且 AUC 附 95% CI）作为 V2-D1 验收框架输入；内部工程验收暂定步速 MAE ≤ 0.05 m/s、夜间心率 MAE ≤ 3 bpm、呼吸率 MAE ≤ 2 bpm、床边离床召回 ≥ 95%、语音只在合格 SNR 和明确说话人下输出。
4. 设备边界维持 CS-C6c-V101-1J4WF + CS-EP-SDNL1 不变；能力矩阵覆盖结论已同步修正。
5. 本次修正为范围与口径修正，不改变任何既有 E1 工程结论、证据等级或模型候选状态。

### 行动项

| 行动 | 负责人 | 截止日期 | 状态 |
|---|---|---|---|
| 修正指标映射、Backlog、命名规范、验证口径与设备矩阵同步 | 项目组 | 2026-08-02 | Closed；本次提交 |
| 获取比赛方案原文并入库，对照一票否决条款复核验收门 | 项目组 | 2026-08-09 | Open |
| 萤石书面确认 SDNL1 开放字段、粒度、留存与事件回调时延 | 设备负责人 | 2026-08-09 | Blocked on vendor contact |
| V2-D1 按三层验证口径设计验收矩阵 | 工程负责人 | 2026-08-12 | Open |
| 隐私方案补齐单独同意文本、PIPIA 与访客告知机制 | 项目组 | 2026-08-20 | Open |

### 未决问题

1. 步态受控任务（3～4 米步行区、TUG、5 次坐站）是否进入 V2 演示范围，若进入需要多少标定与场地工作量？
2. 语音固定问答/图片描述任务由谁触发、老人配合度如何保证；不说话人确认失败时的降级策略是什么？
3. 外部评审引用的实测准确率/可复现性/隐私合规一票否决条款原文尚未入库，V2-RC 门是否需要按原文进一步收紧？

---

## REV-031 双视角基线与无主动测试约束 Review

- 日期：2026-08-02
- 状态：Accepted；Supersedes REV-001 决定 6 的单摄像头边界
- 参与人：项目组
- 评审范围：设备边界升级、双机位安装与融合路线、硬件能力表述边界、无主动测试约束、验证定位降级
- 输入材料：项目组双视角方案（安装方式、融合逻辑、质量指标、预测目标）、外部指标可行性评审、[设备能力矩阵](device-capability-matrix.md)、[信息采集探索](v1-information-acquisition.md)

### 发现

1. 单 RGB 在自然居家步态上受脚部遮挡、视角和透视压缩限制明显；双视角可改善可测率、左右对称性、坐站转换和跌倒检出，但两台消费级相机无硬件同步，不能包装成"高精度双目三维相机"。
2. C6c 同系列接口协议为萤石云私有协议，不能默认 RTSP/ONVIF 直接拉流；能否取得主码流、可靠帧时间戳和长期稳定取流是关键工程风险。
3. 官方同系列参数为"网传帧率自适应"，不能预设恒定 25 fps；双支撑等短时间相受约 40 ms 帧间隔、运动模糊和关键点误差叠加限制。
4. SDNL1 心率/呼吸率按分钟周期上报（官方用户指南），不能声称 50 ms 级原始生命体征；公开文档只有 APP 展示，未找到该型号公开数据 API。
5. 老人家庭验证不安排任何主动步行或睡眠测试；TUG、规定路线、口令任务均移出采集范围，相机标定、对时和安装验收由工作人员完成。
6. 无独立参照硬件时不能声称与动捕、压力步道或 PSG 的金标准精度验证；研究定位为"无感监测可行性与纵向稳定性验证"。
7. 前瞻性跌倒预测受真实事件标签和纵向数据限制，双 RGB 主要降低测量噪声；内部验证目标 AUC 0.75～0.82 现实，0.90 以上不可信。

### 决定

1. 设备边界升级为 2 × CS-C6c-V101-1J4WF + 1 × CS-EP-SDNL1；家庭内只部署这三台设备。C6Wi、H3、RGB-D、压力步道等方案全部排除。
2. 冻结双机位安装方式：主相机侧视主要行走方向，辅相机前后错开形成 30°～60° 斜前/斜后视，2.5～4 米共同有效区域，分别四点以上地面标定；两台均关闭自动巡航、人形跟踪和数字变焦；镜头不正对，避免夜间红外补光互相干扰。
3. 冻结融合路线：两路独立测量 + 事件级时间对齐 + 质量择优 + 结果融合（两路一致为高置信、单路清晰为中置信、差异过大不输出精确空间指标、日/周取中位数）；仅在确认可靠 PTS、固定分辨率和稳定外参后才研究同步三维重建。
4. 冻结硬件能力表述边界（设备能力矩阵 1.1）：C6c 按 400 万/2K+ 档，不按 4K 宣传、不预设恒定 25 fps、不承诺任意方向精确测量、不假定 RTSP/ONVIF；SDNL1 分钟级心率/呼吸率，不测血氧/血压/心电，不诊断呼吸暂停，睡眠报告不等同 PSG，不作步态测距雷达。
5. 步态指标全部来自自然行走片段的自动发现与质量筛选；不安排主动测试类项目（TUG、规定路线步行、5 次坐站口令、单腿/闭眼站立、主动睡眠测试）。
6. 冻结验证降级口径与双视角质量指标（两路步速差、两路步频差、左右脚关键点一致性、跨视角行走事件匹配率、单侧遮挡率、双侧共同有效步数、相机位置漂移量、两路时间戳偏差、双路一致跌倒置信度），写入信息采集探索第 7 节。
7. SDNL1 数据接入以萤石书面确认服务器接口、字段、时间粒度、历史导出、授权和费用为前置。

### 行动项

| 行动 | 负责人 | 截止日期 | 状态 |
|---|---|---|---|
| 双机位/能力边界/无主动测试/验证降级写入信息采集探索、设备矩阵、架构与 README | 项目组 | 2026-08-02 | Closed；本次提交 |
| M2c 采集规程扩展双机位场景（共同区域、标定、漂移检查） | 工程负责人 | 2026-08-09 | Open |
| 实机验证主码流获取、帧时间戳、云端路径与长期取流稳定性 | 设备负责人 | 2026-08-09 | Blocked on device access |
| V2 数据契约增加双路事件对齐与质量择优字段 | 工程负责人 | 2026-08-12 | Open |

### 未决问题

1. 两路 C6c 取流是否共享同一免费账号 3 路并发/1M 码率额度，双路长期运行的费用模型如何？
2. 双机位下已冻结 G4 candidate policy 是否需要按视角分别验证阈值？
3. 老人家庭实际户型是否都能提供 2.5～4 米共同有效区域的安装条件？

---

## REV-032 睡眠组件仓文档形态确认与 E1 预开发 Review

- 日期：2026-08-02
- 状态：Accepted for E1 tooling；账号实测仍 Open
- 参与人：项目组
- 评审范围：萤石设备云组件仓睡眠接口形态、文档族归属、synthetic fixture、候选 mapping、fail-closed 语义保持
- 输入材料：[赛事平台材料归档](v1-m1-ezviz-platform-competition-notes.md)、help/1836～1844（huayi 统计/属性历史）、help/2097（EP/whst 睡眠报告）、[睡眠字段路线](v1-m3-sleep-field-route.md)

### 发现

1. 睡眠雷达接口不止"心率/呼吸率"：文档确认每日统计含**每 10 分钟**心率/呼吸率序列、分期时间线和清醒次数等指标集；属性历史给出 heartRate/breathRate/state/ts 属性级序列；EP/whst 睡眠报告含上床/入睡/醒来/起床、在床/清醒/浅睡/深睡时长、离床次数与各次离床时间、体动、睡眠打分和**每 5 分钟**心率/呼吸频率记录（freqRecordOutput），并带 resultCode 可信度标记。
2. 存在两个文档族：huayi 转发族（deviceId 寻址）与 whst/EP 族（deviceSerial 寻址）；"睡眠伴侣EP"与 CS-EP-SDNL1 的 EP 段一致，但型号兼容归属不能仅凭命名断定，须账号实测。
3. 文档示例存在字段语义矛盾：每日睡眠统计"睡眠时长"单位标"小时"而示例值 31380 更接近秒；单位/时间/缺失语义必须在真实响应上复核。
4. 50 ms 级原始雷达数据仍官方确认不开放；组件仓只提供统计与属性级数据。
5. 现有 profiler 只取最大记录列表：每日睡眠统计的 name/value 指标集和 EP 报告的 timeOutput 编码列表在自动 profile 中不能直接给出语义字段，需真实响应到达后人工判读；`name` 键按敏感键规则不产生候选。

### 决定

1. 接口形态归档进[赛事平台材料归档](v1-m1-ezviz-platform-competition-notes.md)（v1.2），作为账号实测的对照基线。
2. 接受四个 synthetic fixture、候选 mapping 示例 `sdnl1-field-map.component-warehouse.example.json` 和 profiler 别名补充（`ts`、`breathRate`）作为 E1 预开发；fixture 只能产出 candidate，不解锁 adapter。
3. 睡眠路线 policy（19 direct / 5 derived / 11 not-assumed）本次不变；离床次数、睡眠打分、异常心率计数等文档新见字段是否入 policy，待账号实测确认字段语义后再议。
4. 账号实测清单固定为：适用文档族与型号兼容性、每日统计字段清单与单位语义、属性历史/freqRecordOutput 实际粒度、调用频率限制与费用。

### 验证

- 自动化：`tests/test_sleep_component_schema.py` 6 个新测试通过；全量 176 passed。
- fail-closed：四个 fixture 的 route 评估均为 `interface_only_waiting_for_e2_e3_schema`、`direct_ready=0`、`derived_enabled=0`、`values_persisted=false`；合成值不进报告。

### 行动项

| 行动 | 负责人 | 截止日期 | 状态 |
|---|---|---|---|
| 组件仓 fixture + mapping 示例 + 别名 + 回归测试 | 项目组 | 2026-08-02 | Closed；本次提交 |
| 赛事账号内实测睡眠组件仓四项清单 | 设备负责人 | 2026-08-09 | Blocked on platform account |
| 实测后决定是否扩展 sleep route policy 字段 | 工程负责人 | 2026-08-12 | Open |

### 未决问题

1. huayi 转发族与 whst/EP 族是否覆盖同一硬件，还是按固件/销售批次区分？
2. 属性历史与 freqRecordOutput 的实际写入频率是否与文档一致（分钟级 vs 5/10 分钟）？
3. 组件仓功能点是否需要单独报备或付费套餐才能调用？

---

## REV-033 单机位边界与 SDHY1 型号变更 Review

- 日期：2026-08-04
- 状态：Accepted；Supersedes REV-031 的双机位基线（降级为单机位，双机位保留为扩展项）
- 参与人：项目组
- 评审范围：实际到位设备、睡眠仪型号变更、单机位推进策略、9053 未决期间的工作排序
- 输入材料：2026-08-04 交互式 API 实测记录（[赛事平台材料归档](v1-m1-ezviz-platform-competition-notes.md) §10～§12）、设备列表/能力集响应、[设备能力矩阵](device-capability-matrix.md)

### 发现

1. 实际到位并接入账号的设备为 1 × CS-C6c-V101-1J4WF + 1 × **CS-EP-SDHY1**，而非 REV-031 假定的 2 × C6c + 1 × CS-EP-SDNL1；第二台 C6c 未到位。
2. SDHY1 的 huayi 族接口实测可用：每日心率统计（10 分钟序列）与属性历史（约 1 秒间隔、state 字符串枚举）均返回真实数据；睡眠侧数据通路已通。
3. 摄像头套餐经激活码 API 激活成功（activeCode=0），但 HLS 标准流仍返回 9053 占位段；ezopen 地址可生成。9053 原因未决（标准流计费/传播延迟/套餐类型）。
4. 设备能力集实测：音频开关/设备声音/对讲均支持，四档清晰度，SD 卡录像与回放下载支持，云台/畸变校正支持。

### 决定

1. 设备边界确定为 1 × CS-C6c-V101-1J4WF + 1 × CS-EP-SDHY1；REV-031 的双机位安装/融合设计保留为扩展项，待第二台设备到位后重开。
2. 睡眠仪型号由 CS-EP-SDNL1 变更为 CS-EP-SDHY1：sleep route policy 与全部 mapping 示例的 device_model 同步更新，policy_version 升为 v1-sleep-route-2026-08-04；SDHY1 心率 45～150 bpm、呼吸率 0～36 次/分（厂商口径 AP8/AP3>90%）。
3. 9053 未决期间工作排序：睡眠 E2 证据链先行；摄像头取流待代金券/余额核查、控制台画面验证或传播延迟结束后复测。
4. 单机位下步态与跌倒阈值验证按单视角进行；REV-031 冻结的无主动测试约束与验证降级口径不变。

### 验证

- 自动化：型号变更后全量 176 passed（sleep route 测试同步更新 device_model）。

### 行动项

| 行动 | 负责人 | 截止日期 | 状态 |
|---|---|---|---|
| 边界/policy/矩阵/文档同步为单机位 + SDHY1 | 项目组 | 2026-08-04 | Closed；本次提交 |
| 9053 排查：控制台领代金券/查余额、云直播画面验证，必要时工单 | 设备负责人 | 2026-08-06 | Closed；激活码 API 激活 + 传播后解除（2026-08-04，run `20260804T075947Z`） |
| 睡眠真实 E2 导出 → profile/route assessment | 工程负责人 | 2026-08-05 | Closed；run `20260804T075034Z` / `20260804T075035Z`，三 P0 字段 blocked_semantics 仅缺 missing_semantics |
| 套餐激活后复测主码流分辨率/帧率/音轨/PTS | 工程负责人 | 2026-08-06 | Closed；FLV HEVC 2560×1440@15 + AAC 16 kHz mono，capture/probe 双 ready（2026-08-04） |

### 未决问题

1. 9053 的确切含义与解除条件（标准流计费、传播延迟或套餐类型）。
2. SDHY1 属性历史中 state 枚举的完整值域与缺失值表达（STATIC 之外的取值）。
3. 每日睡眠统计需要多少晚数据才返回完整结构；EP/whst 族是否也覆盖 SDHY1。

---

## REV-034 C6c 真实取流、资格门与真机多模态首跑 Review

- 日期：2026-08-04
- 状态：Accepted for E2 toolchain verification；场景矩阵/长稳/远场仍 Open
- 参与人：项目组
- 评审范围：9053 解除、FLV 主码流、三次独立开流资格门、首个真机多模态 run、C01 空房样本
- 输入材料：runs `20260804T075947Z`、`20260804T080037Z`、`20260804T081325Z`、`20260804T081528Z`、`20260804T081613Z`；[赛事平台材料归档](v1-m1-ezviz-platform-competition-notes.md) §14～§16

### 发现

1. 9053 在激活码 API 激活（activeCode=0）并完成传播后解除；此前"512px 无音轨"均为 9048 占位段假象，不是真实码流属性。
2. FLV（protocol=4、quality=1、supportH265=1）给出主码流 HEVC 2560×1440@15fps + AAC 16 kHz 单声道；实际帧率 15 而非 25，"网传帧率自适应"实测成立。
3. 三次独立开流 3/3 ready、轨道签名一致；真实码流格式稳定性首次验证。
4. 真机多模态链路（same_container_pts → YOLO26n + FunASR → 融合窗口）在真实 C6c 容器上跑通；空房 clip 124 帧零人物误激活、VAD 零误报，构成 M2c C01 首个真机负样本。

### 决定

1. 接受上述五个 run 作为 C6c 取流链路的 E2 工具验证证据；矩阵对应行升 E2。
2. C01（白天/近/空房静态）标记为已取得；C02～C10 按采集规程由现场人员配合拍摄，C11/C12 维持条件允许项。
3. 实际帧率 15fps 写入步态/时间相分析口径（REV-031 的 40ms 粒度修正为约 67ms）。
4. 资格门只证明计划性重复开流稳定；断线恢复、长稳、夜视、远场语音、RTMPose 对照保持 Open。

### 验证

- 五次 run 均 completed、owner-only；报告不含 token/序列号/身份信息。
- 自动化：全量 176 passed（本日无代码变更）。

### 行动项

| 行动 | 负责人 | 截止日期 | 状态 |
|---|---|---|---|
| C02～C10 现场拍摄（含双拍手同步事件） | 现场人员 | 2026-08-08 | Open |
| 夜视 C08～C10 与红外画质评估 | 现场人员 | 2026-08-08 | Open |
| RTMPose 变体在 C01 及后续场景对照 | 工程负责人 | 2026-08-09 | Open |
| 30 分钟 wall/media 双门长稳 session | 工程负责人 | 2026-08-09 | Open |

### 未决问题

1. 主码流是否可切 H.264（supportH265=0）以兼容更多解码端；FLV 与 HLS 的轨道差异是否仅容器层。
2. 15fps 是否为当前网络下的自适应结果，固定网络后能否稳定 25fps。
3. 空房误激活为零是在当前机位/光照下成立，换机位后需重测 C01。
